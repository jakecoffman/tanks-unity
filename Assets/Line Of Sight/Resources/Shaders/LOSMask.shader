Shader "Hidden/Line Of Sight Mask"
{
	CGINCLUDE
		...
		float CalculateBackfaceFade(float4 pixelWorldPos, float3 pixelViewNormals)
		{
			...
			float3 directionView = mul((float3x3)UNITY_MATRIX_V, directionWorld);
		...
		float4 SampleAndDecodeDepthNormal(VertexOut i)
		{
			float depth;
			float3 pixelViewNormals;
			float4 encodedDepthNormals = tex2D(_CameraDepthNormalsTexture, i.uv);
		...
		{
			float4 normalDepth = SampleAndDecodeDepthNormal(i);
			float4 positionWorld = DepthToWorldPosition(normalDepth.w, i);
